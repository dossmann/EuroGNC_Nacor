...
Reducing structural loads on an aircraft by advanced control techniques is a main research interest of today's aircraft industry. Reducing the gust loads allow the aircraft manufacturer to build and certify the aircraft for a smaller load envelope, which inherently reduces the structure of the aircraft and reduced costs.
The loads itself arise from steering the aircraft (maneuver loads) and from external disturbance inputs, (gust loads). The latter are considered in this paper for a generic business jet aircraft.

Figure \ref{fig:toolchain} summarizes the proposed tool-chain  with the  herein presented advanced design techniques highlighted in blue. Staring from a complex high order model, a low order approximation is derived using an advanced model approximation technique presented in section \ref{sec:mr}. The derived low order approximations are then used to design a robust disturbance observer for the whole flight envelope (section \ref{sec:th}). a dedicated stability assessment is proposed (section \ref{sec:st}) to finally be able to verify the developed control system in simulation and later validate it in flight test. The presented toll-chain is applied to a modern business jet aircraft in section \ref{sec:app} in this paper and the encouraging results are reported.

\begin{figure}[bth]
	\centering
	%\input{closedloop.tikz}
	\caption{Proposed tool-chain using advanced mathematical tool for developing a gust load alleviation control system.s}
	\label{fig:toolchain}	
\end{figure}
...
As mentioned in section \ref{sec:mr}, the considered aircraft models naturally embed internal and input-out delays. Looping outputs to inputs through an observer/controller structure then leads to dynamical models with many internal delays (i.e. internal plus input-output). Although it is standard and preferred by the users to work on a finite-order approximate model $\hat H$ for the observer and control law design (allowing using standard numerical tools), it is obviously better to assess the stability on the original delayed model before running simulation. Obviously, depending on the number of delays and states equations describing the dynamical model, this assessment can become complicated. As in our considered work, we deal with multiple delays and large amount of state, up to author's knowledge, no  standard tool are available to treat it. In this section we suggest a procedure (with partial theoretical proof) to approximate the stability of a large-scale multiple-delay dynamical equations. 

\subsection{Context and preliminary considerations}

Stability of a dynamical systems is clearly one of the main property to assess in control theory, numerical simulation, optimization, etc. Without loss of generalities, in the case of "classical"\footnote{By "classical", ones means equipped with the $(E,A,B,C,D)$ realization.} LTI models either described by a set of ODE or DAE, the stability problem is recast as the eigenvalues one. In this specific case, the number of eigenvalues is finite and its computational complexity is only related to the $(E,A)$ pencil calculation\footnote{In this case, very efficient tools already exist such as \textbf{LAPACK}.}. If instead, the LTI model $H$ has an infinite number of singularities or its realization is not necessarily available, the stability assessment becomes much more tedious. Delay invariant models fall within this category. In these cases, tailored solutions are usually invented to deal with these specificity (\eg the time-delay stability analysis literature is very important and one may refer to \cite{Richard:2003,Sipahi:2011,Seuret:2015,BriatBook:2015}).  Let us start by noticing that, delayed equations are meromorphic (real-valued) complex functions given as $H :  \Cplx \mapsto   \Cplx^{n_y\times n_u}$. Moreover, if $H$ ensures 
\begin{eq}
\int_{-\infty}^\infty  \norm{\Htran(s)}_F^2 dt <\infty,
\end{eq}
then the meromorphic function is said to be of finite energy and $H\in\Ltwo$. Then, let us define the input-output stability as follows: a system represented by the transfer function $H(s)$ is said to be input-output $L_{\infty} - L_{2}$ stable, if there exists a $c>0$ such that: 
\begin{eq}
\norm{H u}_{L_\infty} =  \norm{y}_{L_\infty} \leq  c \norm{u}_{L_2}.
\end{eq}
In this case, the system is said $\Ltwo$ stable (or $\Htwo$). This consideration, i.e. the complex-valued meromorphic function context, is the starting point for the proposed approach. We start with the derivation of the proposed numerical procedure, summarized in Algorithm \ref{algo:MFSA}. Then, in a second step, theoretical arguments will be given to justify such a procedure.


\subsection{The proposed approach}

%The proposed procedure is summed up as in Algorithm \ref{algo:MFSA}.

\begin{algorithm}
\caption{\mfsa - $\Ltwo$ Meromorphic Function Stability Approximation} \label{algo:MFSA} 
\begin{algorithmic}[1]
...
Algorithm \ref{algo:MFSA} embeds a relative simple procedure, but also quite effective, fast and reliable in multiple cases. In short words, the idea consists in exactly matching the original input-output model by a rational model $\tilde H\in\Ltwo$, by guaranteeing interpolatory conditions. Then, to seek for the best stable approximation $\tilde H_s\in\Htwo$ of the obtained model $\tilde H \in\Ltwo$. The $\Ltwo$ distance (inner product) between the interpolated $\tilde H$ and stable $\tilde H_s$ models is then computed. If this latter is smaller than a given threshold, then one concludes that $\tilde H$ (and consequently $H$) is stable, and unstable otherwise. Obviously, such a simple procedure deserves some arguments  to be fully satisfactory.  In the following we derive some of them to justify such an approach. Still, author stresses that all the necessary ones are not given yet, and would require deeper studies. 


\subsection{Reminding remarks on stability by $\Ltwo$ inner product}

Let us consider a \mimo linear dynamical system, denoted by $H$ with $n_u$ (\emph{resp.} $n_y$) $\in\mathbb{N}^{\ast}$ inputs (\emph{resp.} outputs), represented by its transfer function $h(s)\in \Cplx^{n_y\times n_u}$. Let $\Ltwo(\imath\Real)$ be the Hilbert space of holomorphic functions ${F}:~\Cplx\rightarrow \Cplx^{\nynu}$ which are analytic in the complex plane except on the imaginary axis and for which 
\begin{eq}
\int_{-\infty}^{+\infty}\trace\left(\overline{ F(\imath\omega)} F^T(\imath\omega)\right)d\omega<+\infty. 
\end{eq}
...
\end{eq}
which maps the causal and anti-causal time-domain functions. Obviously, \eqref{eq-Stability:laplaceEquivalence} shows that every element $H\in \Htwo(\Cplx_+)$ (respectively ${G}\in \Htwo(\Cplx_-)$ ) can be uniquely associated to an element  $\mathbf{h}\in \mathcal L_2^n[0,\infty)$ (respectively $\mathbf{g} \in L_2^n(-\infty,0] $). In addition, the following functional analysis theorem shows that the Laplace transform preserves inner product and orthogonality.

\begin{theorem}[Plancherel] \label{thm:Stability-Plancherel}
Let us consider $\mathbf h_1, \mathbf h_2 \in \mathcal L^n_2(-\infty,\infty)$, one has
\begin{eq}
\langle H_1 , H_2 \rangle_{\Ltwo(\imath\Real)}=  \langle  \mathcal L(\mathbf h_1) , \mathcal L (\mathbf h_2)\rangle_{\Ltwo(\imath\Real)} = \langle \mathbf h_1 , \mathbf h_2\rangle_{L_2}.
\end{eq}
Moreover, since $\Htwo(\Cplx_-)$ is orthogonal to $\Htwo(\Cplx_+)$ with respect to the $\Ltwo(\imath\Real)$-inner product, if $H_s \in \Htwo(\Cplx_+)$ and $H_a = \Htwo(\Cplx_-)$, then
\begin{eq}
\langle H_s, H_a \rangle_{\Ltwo(\imath\Real)} =0.
\end{eq}
\end{theorem}


In other words, the above decomposition and Theorem \ref{thm:Stability-Plancherel} state that given a model $\Htran\in\Ltwo(\imath\Real)$, there is a stable model  $H_s \in \Htwo(\Cplx_+)$ and an anti-stable model  $H_a = \Htwo(\Cplx_-)$ such that $H = {H}_s +H_a$ and $\langle H_s, H_a \rangle_{\Ltwo(\imath\Real)} =0$. Then the following proposition holds.
...
In other words, if a system $H$ is stable, then the global minimizer $\tilde H$ of $\Ltwo$ approximation problem is stable too. In the same way, if a system $H$ is anti-stable, \ie all its singularities are unstable, then the global minimizer $\tilde H$ of the $\Ltwo$ problem is anti-stable as well. This result comes from the orthogonality property of $\Htwo(\Cplx_-)$ and $\Htwo(\Cplx_+)$ spaces. 
...
In other words, the set $\Htwo(\Cplx_+)$ is not an open set of $\Ltwo(\imath\Real)$. As a consequence, it is always possible to approximate a stable model $H$ by an unstable one of order $k$ while decreasing the mismatch error $\norm{H -\tilde H_k}_{\Ltwo}$. Similarly, let us now consider the case where the initial model $H$ both has stable and unstable modes. 
...
In other words, the set of unstable systems $\Ltwo(\imath\Real)\backslash \Htwo(\Cplx_+)$ is an open set of $\Ltwo(\imath\Real)$. Moreover, by fixing an arbitrarily small $\varepsilon$, it is always possible to find a $\tilde H_k$ that is unstable too. Based on the above propositions, let us now formulate the stability argument which will be invoked in order to derive the proposed numerical procedure.
...
In \cite{PontesECC:2015}, these arguments were used to derive a procedure based on the \tfirka algorithm \cite{BeattieCDC:2012}, combined with a search of the approximation order $r$. This last procedure did provide quite good results but the search for an adequate order $r$ was complex. Moreover, the \tfirka is an $\Htwo(\Cplx_+)$-oriented procedure and its validity in the $\Ltwo(\imath\Real)$ function space is limited to models where the stable and anti-stable part is known \cite{Magruder:2010}. 

Here, the $\Htwo$-optimal interpolatory conditions are released and one now considers the interpolatory conditions embedded in the Loewner framework instead. One major benefice of such a trade stands in the selection of the approximating order $r$, which may be automatically selected thanks the Loewner matrix rank computation By coming back to the \mfsa procedure defined in Algorithm \ref{algo:MFSA}, step 2 provides a quite simplifying solution, where the dimension $r$ is automatically determined. Then, following Propositions \ref{propStability:stable} and \ref{propStability:unstable}, it appears quite natural that, approximating the interpolated model $\tilde H$ with $\tilde H_s$ using the methodology proposed by \cite{Kohler:2014}, followed by the $\Ltwo$ distance evaluation is appealing. Indeed, as 
\begin{itemize}
\item From a stable model it is always possible to find an unstable one which minimizes the $\Ltwo$ mismatch problem, if, based on an unstable approximation, a same complexity (order) stable model may be obtained, without affecting the $\Ltwo$ norm, then the interpolated model is stable, as the original one.
\item From an unstable model, its global minimizer in the $\Ltwo$ sense should be unstable as well, then, applying a stable approximation will lead to a large $\Ltwo$ mismatch. Then, one may confirm that the original model is unstable, as its rational approximant.
\end{itemize}
...
Ten LTI aircraft models, each of about 300 states, linearized on 10 different trim points in the flight envelope, i.e. different speeds and altitudes, and weight and balance domain, i.e., different masses and center of gravitz positions, are available.
...
Let us now apply the proposed \mfsa procedure to the considered aircraft models including two internals and one output delays. As the above observer and a static gain feedback are used in the loop, we now are interested, before running multiple simulations, in evaluating the stability of this $k$-dependent multiple delays model. The model reads as:
\begin{eq}
	H^{(i)}(s,k) = \C^{(i)}\big(s E^{(i)}-\A^{(i)}_0-\A^{(i)}_1e^{-\tau_1s}-\A^{(i)}_2e^{-\tau_2s}-\A^{(i)}_3e^{-\tau_3s}\big)^{-1}\B^{(i)},% + D^{(i)}.
	\label{eq:stabilityTest}
\end{eq}
where $\tau_3$  stands as the additional output delay caused by the acquisition chain and $k\in\Real_-$ is the controller gain (here we use a static gain for simplicity) which will be used as tuning parameter to evaluate the closed-loop stability before time-domain simulations (note also that the row dimension of the above matrices is around 500). Figure \ref{fig:stability} illustrates the results of the \mfsa when applied to \eqref{eq:stabilityTest}, for different frozen values of $k$, between $-2.5$ and $-0.1$ (here 50 points linearly spaced are used).
...
With reference to Figure \ref{fig:stability}, it appears that a gain $k<-0.7571$ will result in a large index $S$, meaning that the closed-loop system is unstable. Then for $-0.6143<k<-0.1$, the index becomes low, meaning that this latter configuration will result in stable closed-loop. Interestingly, when time-domain simulations are performed (not given for confidentiality) for these frozen $k$-values, similar observations are obtained, which is very satisfactory.

However, when $k$ is in between $-0.7571$ and $-0.6143$, the stability index is rather low then high, meaning that the closed-loop should be stable then unstable. However, within this $k$-interval, the closed-loop is always unstable. Such a problem is not solved yet, but additional studies are on-going to point the issue occurring between $-0.7571$ and $-0.7$ (nevertheless, reader should consider the multiple source of mistake, including numerical aspects arising in the large scale setting). 
