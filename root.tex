...
\begin{equation}\label{eq:sys}
	y^{(i)}(s) = G_u^{(i)}(s) u(s) + G_{n}^{(i)} n(s)  + G_d^{(i)} d(s),
\end{equation}
...
\begin{figure}[bth]
	...
	\sidecaption[]
	\input{gusts.tikz}
	\caption{Gust estimates (\ref{lin:1}) on the 10 available trim points for a gust input (\ref{lin:ref}) with a wavelength of 27.5\,m (first diagram), 34.7\,m (second diagram), and 46\,m (third diagram) }
	\label{fig:est}.
\end{figure}
...
Finally, the loads on the wings are computed without gust load alleviation controller and with. For each of the 10 trim points five different gust wavelength are simulated and the accordant loads are recorded. The bending moment value for each trim point is depicted in figure \ref{fig:loads} in dependence of the wing position. The same procedure is repeated with the estimator and the feedback law activated.
