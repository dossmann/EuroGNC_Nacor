...
Finally, the 10 aircraft models each of about 300 states with nine disturbance inputs have been approximated with ten models each of order $r=25$ with a single disturbance input. These reduced models can now be used to design the disturbance estimator. 
...
For the design of the estimator $O$  the ten reduced order models derived in section \ref{APPsubsec:model} are used 
...
\begin{figure}[bth]
	...
	\sidecaption[]
	\input{gusts.tikz}
	\caption{Gust estimates (\ref{lin:1}) on the ten available trim points for a gust input (\ref{lin:ref}) with a wavelength of 27.5\,m (first diagram), 34.7\,m (second), and 46\,m (third). }
	\label{fig:est}
\end{figure}
